...
SRC = ROOT / 'results_v_lit_eta'
OUT = ROOT / 'results_v_lit_eta_figures'
...
def fig1():
    ...
    for ax, d1, d2, title, y2label in [
        (axes[0], b_delay, b_comp, '(a) Lit-eta Base Case', 'Completion rate [%]'),
        (axes[1], c_delay, c_urgent, '(b) Lit-eta Primary Challenge', 'Urgent on-time completion [%]')]:
    ...
    fig.suptitle('Figure 1. Lit-eta Base vs Primary Challenge with corrected C4 and C5 15-min RH-MILP benchmark', y=1.02, fontweight='bold')
...
if __name__ == '__main__':
    ...
    print(f'Generated literature-eta figures in {OUT}')
