...
fig,ax=plt.subplots();cfg=load_final_wpt_configuration();
for power in (1,3,5): cfg['wpt_power_kw']=power;ax.plot(np.linspace(0,175,8),physical_efficiency_values(cfg)*100,label=f'{power} kW')
ax.legend();ax.set(xlabel='Misalignment [mm]',ylabel='Efficiency [%]');save(fig,'Figure5_wpt_efficiency')
